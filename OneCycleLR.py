...
class OneCycleLR(_LRScheduler):
    ...
    Args:
        ...
        pct_warmup (float): The percentage of the cycle (in number of steps) spent
            ...
            Default: 0.1
        pct_down (float): The percentage of the cycle (in number of steps) spent
    ...
    Example:
        ...
        >>> optimizer = torch.optim.SGD(checkpoints.parameters(), lr=0.1, momentum=0.9)
    ...
    def __init__(self,
        ...
        if three_phase:
            self._schedule_phases = [
                {
                    'end_step': float(pct_warmup * self.total_steps) - 1,
                    'start_lr': 'initial_lr',
                    'end_lr': 'max_lr',
                ...
                {
                    'end_step': float((pct_down+pct_warmup) * self.total_steps) - 2,
                    'start_lr': 'max_lr',
                    'end_lr': 'best_lr',
        ...
        # Validate pct_start
        if pct_warmup < 0 or pct_warmup > 1 or not isinstance(pct_warmup, float):
            raise ValueError("Expected float between 0 and 1 pct_start, but got {}".format(pct_warmup))
        ...
        if last_epoch == -1:
            for idx, group in enumerate(self.optimizer.param_groups):
                ...
                group['best_lr'] = max_lrs[idx] / div_factor
                group['min_lr'] = group['best_lr'] / final_div_factor
